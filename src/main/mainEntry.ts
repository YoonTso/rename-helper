...
async function restoreAll(files: string[], separator: string, cb: Function) {
    files.forEach(async (item) => {
        const stats = await stat(item);
        if (stats.isDirectory()) {
            const newName = (await restoreFolder(item, separator)) as string;
            const files = await readdir(newName);
            const childFiles = files.map((item) => path.join(newName, item));
            await restoreAll(childFiles, separator, cb);
        } else {
            const newName = await restoreFile(item, separator);
            cb(item, newName);
        }
    });
}
...
async function restoreFolder(
    ...
    try {
        const { dir, name } = path.parse(folderName);
        const newFolderName = name.split(separator).join("");
        const newName = path.join(dir, newFolderName);
        await rename(folderName, newName);
        return newName;
    } catch (err) {
...
async function restoreFile(
    ...
    try {
        const { dir, name, ext } = path.parse(fileName);
        const newFileName = name.split(separator).join("");
        const newName = path.join(dir, newFileName + ext);
        await rename(fileName, newName);
        return newFileName;
    } catch (err) {
...
ipcMain.on("restore", async (event, files: string[], separator: string) => {
    await restoreAll(
        files,
        separator,
        function (oldName: string, newName: string) {
            event.sender.send("onRestore", oldName, newName);
        }
    );
});
